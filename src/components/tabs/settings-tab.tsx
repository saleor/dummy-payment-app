...
const WEBHOOKS = [
  ...
  { name: "TRANSACTION_INITIALIZE_SESSION", description: "Initial transaction response with configurable event type" },
  { name: "TRANSACTION_PROCESS_SESSION", description: "Process-step response with configurable event type" },
  { name: "TRANSACTION_CHARGE_REQUESTED", description: "Handles charge requests from Saleor" },
  { name: "TRANSACTION_REFUND_REQUESTED", description: "Handles refund requests with amount validation" },
...
export function SettingsTab() {
  ...
  return (
    <Box display="grid" gap={8}>
      ...
      <Box
        display="grid"
        gridTemplateColumns={{ desktop: 3, mobile: 1 }}
        gap={4}
      >
        <StatusCard
          title="Gateway"
      ...
      >
        <Box
          display="grid"
          gridTemplateColumns={{ desktop: 2, mobile: 1 }}
          gap={2}
        >
